...
**文档状态：** 已落地（H5 / 微信小程序统一事件名）
**对应实现：** `frontend/src/services/themeAnalytics.js`

乡声集盒主题中心记录用户浏览、筛选、收藏、分享、预览、启用与获取装扮等行为。只统计行为，不采集昵称、手机号、邮箱、头像、账号 id、openid 等隐私字段。待上线、已绝版装扮同样记录浏览与详情进入。总览见 [`THEME_CENTER.md`](THEME_CENTER.md)。分期交付见 [`THEME_CENTER_ROADMAP.md`](THEME_CENTER_ROADMAP.md)：一期只需进入、切 Tab、启用；完整事件在二期验收。跳转链路见 [`THEME_CENTER_NAV.md`](THEME_CENTER_NAV.md)。性能事件见 [`THEME_CENTER_PERF.md`](THEME_CENTER_PERF.md)。
...
| `theme_tab_switch` | 切换 Tab | `tab`：全局主题 / 局部装扮；二期起可有 我的收藏 / 我的装扮 |
...
| `theme_apply_click` | 启用 / 应用 | `item_id`，`item_type`，`from_history`，`is_mix`，`apply_result`（成功启用 / 权限不足 / 环境不支持），必要时 `permission_type` |
...
运营看板的筛选时间（今日 / 近 7 天 / 近 30 天）、角色与只读用户数据见 [`THEME_CENTER_ADMIN.md`](THEME_CENTER_ADMIN.md)。
