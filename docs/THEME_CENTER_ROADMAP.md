...
本文只定范围、优先级和小程序降级，不定 UI 稿。模块总览、风险与验收见 [`THEME_CENTER.md`](THEME_CENTER.md)。数据契约见 [`THEME_CENTER_DATA.md`](THEME_CENTER_DATA.md)，容错见 [`THEME_CENTER_FAULT.md`](THEME_CENTER_FAULT.md)，埋点见 [`THEME_CENTER_ANALYTICS.md`](THEME_CENTER_ANALYTICS.md)，跳转链路见 [`THEME_CENTER_NAV.md`](THEME_CENTER_NAV.md)，后台运营见 [`THEME_CENTER_ADMIN.md`](THEME_CENTER_ADMIN.md)，性能见 [`THEME_CENTER_PERF.md`](THEME_CENTER_PERF.md)，安全见 [`THEME_CENTER_SECURITY.md`](THEME_CENTER_SECURITY.md)。
...
| 全局主题 | 列表、卡片、详情弹窗、大图预览；启用；可用 / 待上线占位 |
...
1. Tab **我的收藏**；收藏 / 取消收藏。
2. 搜索、筛选、排序；**方言地域标签**筛选。
3. 分享装扮、生成分享海报（小程序相册权限失败给提示）。
4. 最近使用（最多 8 条）；保存搭配最多 10 套；一键应用。
5. 实时模拟预览（模拟首页罐头流 / 个人中心 / 评论区 / 话题卡，不改真实界面，直到点「立即应用」）。
6. 权限：免费、会员专属、活动限定、方言创作者专属（**启用必须服务端校验**，前端置灰不够）。
7. 补齐局部装扮分类：交互按钮、话题卡片、弹窗 / 输入框。
8. 小程序：**可以展示**导航栏、底部 Tab 栏装扮，但必须置灰，写明原生组件不支持，不是故障。
9. 埋点补全；热度 / 点赞仅作排序与展示，不做社交解锁。
10. 登录合并（云端 / 本地 / 合并两者）；完整边界处理（见容错文档）。
...
不在主题中心里做直播装扮、第三方皮肤市场、跨 App 皮肤导出。

### 三期验收闸门

- 碎片与兑换有服务端账本，不能只改本地数字。
- 投稿有审核态，未过审不对 C 端展示。
- 复制他人搭配走二期同一套冲突 / 下架跳过逻辑。
